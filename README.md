...
## day 21 拖拽图片
    ...
    2. 拖拽元素监听:dragstart, dragend, 分别监听t拖拽开始和结束, 使用setTiemout的执行顺序特性来实现拖拽开始后该元素本体消失,只显示拖拽中的
...
## day38 代码账号验证
    为什么要使用定时器：
    首先，我们是希望用户输入后跳转到下一个输入框，要给输入框添加监听器，监听可以分两种情况1.监听input输入情况，2.监听keydown键盘按下；我这边两种都试了，最后决定采用keydown，因为在监听input情况下我暂时没有很好的方案对只能输入一个数字进行判断检测。
    使用keydown可以拿到当前输入的值，然后判断这个值是否可以使用，可以使用就进行赋值然后聚焦下一个输入框，但input本身也是会有输入赋值事件，并且input事件是在keydown之后执行，所有会出现下一个输入框也被赋值，使用定时器，开启异步让赋值和改变焦点操作在input自身的输入之后就可以了。
    发现存在一个诡异的问题：
    明明是数字输入input，其他字符都不能输入，但e可以！。。。